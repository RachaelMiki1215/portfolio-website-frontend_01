...
const baseUrl: string =
  "https://raebux-portfolio-website-web-api.azurewebsites.net";

const getWorkExperienceDynamic = async (): Promise<WorkExperienceType[]> => {
  const res = await fetch(`${baseUrl}/workexperience`, {
...
const getEducation = async (): Promise<EducationType[]> => {
  const res = await await fetch(`${baseUrl}/education`, {
    cache: "force-cache",
  });
